import { AgentCard, AgentCardSchema, TaskNegotiation } from '../schemas/a2a';
...
export class SynapseRouter {
   ...
   */
  async discover(trait: string): Promise<AgentCard[]> {
    return Array.from(this.registry.values()).filter(agent => 
      agent.capabilities.some(cap => cap.trait === trait)
    );
  }
   ...
   */
  async negotiate(negotiation: TaskNegotiation, providerId: string): Promise<boolean> {
